 ...
 *
 * Contributor(s): C_Classic (cclassicvideos@aol.com) - Conversion to C++
 *
...
char LCD::lcd_mode; 		//!< Currently used mode, can be text or graphic.
char LCD::lcd_curline;	//!< Current line. Used for scrolling in text mode.
 ...
 */
void LCD::lcd_init(uint8_t mode) {
 ...
 * @param txt the text
 */
void LCD::lcd_write_text(char *txt) {
uint8_t c,tmp,x,y;
uint16_t pos;
 ...
 *
 */
void LCD::lcd_gotoxy(uint8_t x, uint8_t y) {
uint16_t pos;
 ...
 */
void LCD::lcd_plot_pixel(uint8_t x, uint8_t y, uint8_t set) {
uint16_t pos;
uint8_t xr;
 ...
 *
 * <Sebastian> This function is dedicated to Greta, one of the most important persons in my life so far.\n
 *
 */

void LCD::lcd_plot_bitmap(uint8_t x, uint8_t y, PGM_P bitmap, uint8_t w, uint8_t h) {
 ...
 *
 */
void LCD::lcd_plot_char(uint8_t x_off, uint8_t y_off, uint8_t c, uint8_t fw, uint8_t fh, PGM_P font) {
PGM_P letter;
uint8_t fsize,fwb;
 ...
 * @see lcd_plot_char
 */
void LCD::lcd_plot_text(uint8_t x_off, uint8_t y_off, const char *text, uint8_t fw, uint8_t fh, PGM_P font) {
...
* @see lcd_plot_text
*/
void LCD::lcd_plot_pgmtext(uint8_t x_off, uint8_t y_off, PGM_P text, uint8_t fw, uint8_t fh, PGM_P font) {
    uint8_t c;
